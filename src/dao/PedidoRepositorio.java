...
public class PedidoRepositorio implements Repositorio<Pedido> {
    // Lista interna que almacena todos los pedidos
    private List<Pedido> pedidos = new ArrayList<>();

    // ==================== MÉTODOS C(reate)R(ead)U(pdate)D(elete) ====================
    ...
    public Pedido buscarPorId(String numeroPedido) {
        for (Pedido p : pedidos) {
            // Compara el número de pedido ignorando mayúsculas/minúsculas, así evitamos problemas
            if (p.getNumeroPedido().equalsIgnoreCase(numeroPedido)) return p;
        }
        return null;
    }
    ...
    // Elimina un pedido por su número de pedido
    @Override
    public void eliminar(String numeroPedido) {
        // removeIf elimina todos los pedidos que cumplan la condición
        pedidos.removeIf(p -> p.getNumeroPedido().equalsIgnoreCase(numeroPedido));
    }
